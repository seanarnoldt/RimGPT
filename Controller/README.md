...
state, the decision baseline, and strategic memory. It contains a bounded
assessment and at most six pending, blocked, or deferred open loops. The model
must retain each prior loop or explicitly resolve it as completed, cancelled,
or invalidated after checking current authoritative state.

`finish_decision` is a local terminal CORE tool. It validates the handoff and
ends the decision without another Responses request or any RimWorld command.
New loop IDs are deterministic compact values generated by the controller;
prior IDs are used when retaining, updating, or resolving a commitment.

After final authoritative state retrieval, a successful non-dry-run cycle
commits `decision_handoff.json` and `decision_baseline.json` in the same
colony-scoped StateStore directory. Failed requests, context guards, request
limits, compaction failures, and isolated tool execution do not replace the
last valid handoff. Ordinary final text is supported as a fallback assessment
and conservatively retains existing open loops.

Dry-run handoff output is never persisted. The one-cycle dry-run proposal
ledger remains independent from cross-cycle decision continuity. Context logs
report `decisionHandoffChars`; the hard handoff document limit is 4,000
characters, while normal handoffs are expected to remain below 1,000.
